...
def get_prediction(
    model: FasterRCNN, image: torch.Tensor, device: torch.device, threshold: float = 0.5
...
def process_directory(
    ...
    for img_file in image_files:
        ...
        output_path = os.path.join(
            output_dir, f"{os.path.splitext(img_file)[0]}_detection.jpg"
        )
...
def main():
    ...
    if os.path.isdir(args.input):
        # Process directory
        process_directory(
            model, args.input, args.output, device, config["img_dim"], args.threshold
        )
    else:
        ...
        # Save visualization
        output_path = os.path.join(
            args.output, f"{os.path.basename(args.input).split('.')[0]}_detection.jpg"
        )
        visualize_predictions(args.input, prediction, output_path)

        # Save prediction to JSON
        output_json = os.path.join(
            args.output, f"{os.path.basename(args.input).split('.')[0]}_detection.json"
        )
